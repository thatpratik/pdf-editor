...
2. Render a selectable/draggable overlay box per detected image atop the page canvas (same mechanism as Phase 7's per-block overlay: convert the PDF-space bounding box to on-screen CSS pixels using the same viewport transform), supporting drag-to-move, a corner handle for resize, and a delete action.
3. On commit, dispatch an `APPLY_IMAGE_EDIT` action appending one `PageEdit` — `{ type: 'image', originalBoundingBox, newBoundingBox, imageBytes, imageFormat }` — to that page's `edits` array (`newBoundingBox: null` for a delete). `imageBytes`/`imageFormat` are extracted once, at commit time, from the page's `XObject` resource dictionary (`page.node.Resources()` — this part of pdf-lib's low-level object model genuinely is public and documented) — reading the already-embedded image stream's raw bytes and subtype. Like Phase 7's text edits, this is pure state — no pdf-lib document is touched here, and it's automatically covered by Phase 4's undo/redo the same way.
4. The actual drawing happens later, inside `buildPdf`: after copying the page, for each `'image'` edit, draw an occluding rectangle over `originalBoundingBox`, then — unless `newBoundingBox` is `null` — `embedJpg`/`embedPng` the stored `imageBytes` and `page.drawImage(...)` at `newBoundingBox`'s position/size. This mirrors Phase 7's text approach exactly, and carries the identical, worth-repeating caveat: the original image's bytes remain physically present in the copied page's content stream/XObject dictionary underneath the occluding rectangle, not truly removed — surface this alongside Phase 7's text-occlusion caveat as one combined "what 'editing' really means in this tool" disclosure to the user, not two separate surprises.

**Done when:** an existing image on a real sample page is correctly detected with its actual position/size (verified by drawing the detected overlay box directly on top of the rendered page and confirming visually that it lines up with the real image, before any editing UI is built on top of it), can then be dragged to a new position and resized, the change is reflected in the downloaded PDF, and a delete visually removes it from the output without corrupting the rest of the page's content.
