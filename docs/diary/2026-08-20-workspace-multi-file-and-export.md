...
## Step 7: In-place text editing with local-block reflow (Phase 7)

**Author:** main

### Prompt Context

**Verbatim prompt:** Same standing autonomous instruction as Steps 4-6, plus the explicit Phase 7 checkpoint it built in: before writing any Phase 7 code, an `AskUserQuestion` was raised quoting the plan's own caveat verbatim (neither pdf-lib nor any client-side library can truly delete existing content-stream text — occlusion covers it, but the original bytes remain physically present and recoverable) and offering three options: proceed with an in-app disclosure, proceed without one, or pause to revisit scope. The user chose "Proceed as planned, surface it in-app."
**Interpretation:** Build Phase 7 exactly as `IMPLEMENTATION_PLAN.md` specifies — pdf.js text-block detection, an absolutely-positioned contentEditable overlay per block, edits stored as data (not live document mutations) via a new `APPLY_TEXT_EDIT` action, and the actual pdf-lib drawing deferred to `buildPdf` — plus add a user-facing (not just repo-documented) disclosure of the occlusion caveat, shown when text-edit mode is entered.
**Inferred intent:** Ship the hardest remaining phase to the letter of the plan's design, but close the gap between "the limitation is written down in `IMPLEMENTATION_PLAN.md`" and "the person using the tool actually knows about it" — since the PRD's own audience (a small team doing routine PDF chores) would have no reason to read the engineering plan.

### What I did

- Checked pdf.js's actual typings before writing anything (`node_modules/pdfjs-dist/types/src/display/api.d.ts`, `page_viewport.d.ts`) rather than assuming API shapes from memory: confirmed `getTextContent()` returns `{ items: (TextItem | TextMarkedContent)[], styles: Record<string, TextStyle> }`, that `TextItem` isn't re-exported from the top-level `pdfjs-dist` package (only from its internal `src/display/api` module), and that `PageViewport` only exposes `convertToViewportPoint`/`convertToPdfPoint` in this version's typings (no `convertToViewportRectangle`), which shaped several implementation choices below.
- Added `/src/lib/textBlocks.ts`: `getTextBlocks(doc, pageNumber): Promise<TextBlock[]>`. Since `TextItem` isn't importable by name, defined a local `RawTextItem` interface capturing just the fields needed (`str`, `transform`, `width`, `fontName`) and cast into it after an `'str' in item` runtime filter, rather than fighting a type-predicate assignability error against pdf.js's `TextItem | TextMarkedContent` union. Clusters items into lines (shared baseline Y within a small epsilon) then lines into blocks (adjacent lines with vertical gap below a font-size-relative threshold and roughly-aligned left edges) — the exact two-stage heuristic the plan describes. One small improvement over the plan's literal wording: block font matching uses `textContent.styles[item.fontName]?.fontFamily` (a real, human-readable font family string pdf.js already resolves) rather than the raw internal `fontName` string (something like `g_d0_f1`, uninformative for heuristic matching) — falling back to the raw name only if no style entry exists.
- Added `matchStandardFont(fontFamilyHint): StandardFonts` and a `wrapText(text, font, fontSize, maxWidth): string[]` greedy word-wrapper to `/src/lib/pdfExport.ts`, plus rewrote `buildPdf`'s edit-application step (previously a no-op, since `edits` was always `[]` until this phase) as a new `applyEdit` helper: draws an occluding white rectangle, embeds (and caches, per `buildPdf` call, in a new `embeddedFontsByKey` map alongside the existing per-file `PDFDocument` cache) the matched standard font, then draws the wrapped lines on top.
- Added `APPLY_TEXT_EDIT` to both `WorkspaceAction` and `PagesAction` in `/src/features/workspace/workspaceReducer.ts` (identical shape in both, following the same pattern Step 4 established for `ROTATE_PAGE`/`DELETE_PAGE`) and a `pagesReducer` case that appends the new edit to that page's `edits` array — this alone is what makes text edits undoable "for free," exactly as Step 4's diary anticipated.
- Added `/src/features/workspace/TextEditOverlay.tsx`: one absolutely-positioned, `contentEditable` `<div>` per detected block, positioned by running the block's PDF-space bounding box corners through `viewport.convertToViewportPoint` (twice, for two opposite corners, then taking min/max — since `convertToViewportRectangle` isn't in this version's typings) and multiplying by a `displayScale` factor.
- Extended `/src/lib/pdf.ts` with `getPageViewport(doc, pageNumber, scale, additionalRotation)` — the exact same rotation-compositing computation `renderPageToCanvas` already used internally, now exposed separately so the overlay can convert PDF coordinates through the identical transform the canvas was actually drawn with, rather than re-deriving it and risking drift between the two.
- Rewrote `/src/features/workspace/PagePreview.tsx` to add an "Edit text" toggle, a `viewport` + `displayScale` pair tracked via a `ResizeObserver` on the canvas element, and a `hasSeenTextEditCaveat`/`onDismissTextEditCaveat` prop pair (state lifted to `Workspace.tsx`, since the disclosure should be shown once per session across every page a user might edit, not reset every time the preview component happens to re-render for a different page). Wired a new `onApplyTextEdit` callback prop through to `Workspace.tsx`, which dispatches `APPLY_TEXT_EDIT` with the currently-selected page's id.
- Added the in-app disclosure itself: a dismissible amber banner ("Edited or covered text isn't fully removed from the file...") shown whenever text-edit mode is active and hasn't been dismissed yet this session, with its own explicit "Got it" button — this exact shape (a separate dismiss button, not auto-dismiss-on-open) is deliberate; see "What didn't work" below for why.

### Why

Keeping edits as pure data (`PageEdit` records appended to `WorkingPage.edits`) rather than live pdf-lib mutations — deferring all actual drawing to `buildPdf` — is what lets a single edit action be simultaneously undoable (Step 4's history wrapper needs nothing new to cover it) and safely re-buildable on every download/extract/split call, exactly as the plan's Phase 2 architecture was designed to support from the start. Surfacing the occlusion caveat in-app (rather than only in this diary and the plan) directly follows from the user's explicit choice at the Phase 7 checkpoint, and matters because this is genuinely still an "editor" limitation someone could get burned by (e.g. redacting sensitive text on screen while it remains recoverable underneath).

### What worked

- Reading pdf.js's actual `.d.ts` files before writing code paid off directly: knowing upfront that `TextItem` isn't exported by name and that `convertToViewportRectangle` isn't in this version's typings meant the local-interface-and-cast approach and the two-point min/max conversion were designed in from the start, rather than discovered via a failed `tsc -b`.
- `tsc -b` and `eslint .` both passed cleanly except for one real lint catch (see below) — the reducer/history plumbing extension (`APPLY_TEXT_EDIT`) slotted into the existing `WorkspaceAction`/`PagesAction` split from Step 4 with zero friction, confirming that architecture really does generalize to new edit types as advertised.
- Playwright verification went well beyond a smoke test, and was essential given this phase's own complexity: generated a rectangular (non-square, per Step 3's lesson) single-paragraph sample PDF and a second, purpose-built multi-paragraph sample (a title line, a 3-line paragraph, and a 2-line paragraph, each block separated by a deliberately larger vertical gap) specifically to exercise the block-clustering heuristic the plan calls out as needing review "against a real multi-paragraph sample document, not analytically derived." `getTextBlocks` correctly produced exactly 3 blocks with exactly the right line groupings — the title stayed separate, and each paragraph's lines joined into one block without bleeding into its neighbor.
- The strongest verification was decompressing the actual output PDF's content streams (see "What was tricky" below) and hex-decoding the drawn `Tj` operands, which showed, byte-for-byte: the original text (`File A - Page 1`) still present and untouched in its original content stream object, a white occlusion rectangle at the edited block's position, and the new text correctly wrapped across multiple lines beneath it — direct, physical confirmation both that the feature works and that the disclosed caveat is accurate, not just asserted.

### What didn't work

- `npx eslint .` failed once, on the first cut of the text-block-refetch effect:
  ```
  Error: Calling setState synchronously within an effect can trigger cascading renders
  ...
  react-hooks/set-state-in-effect
  ```
  The offending code called `setTextBlocks(null)` unconditionally inside a `useEffect` whenever `isEditingText` became `false`. Fixed by moving that reset into the `handleToggleEditText` event handler itself (a direct user-triggered update, not an effect body), leaving the effect to only ever fetch-and-set blocks when edit mode turns on.
- A genuinely misleading dead end during manual verification: an initial Playwright script tried to select a contentEditable block's existing text via `page.keyboard.press('Control+a')` before typing a replacement. This did not select anything — the typed replacement text was inserted *before* the original text instead of replacing it (`innerText` came back as `"EDITED TEXT HEREFile A - Page 1"`, the new text glued onto the front of the old). This looked at first like the edit commit path might be broken, so I added temporary `console.log` instrumentation at every hop (`handleCommitBlock` → `onApplyTextEdit` → `buildPdf`'s per-page edit loop → `applyEdit`'s computed box/lines) and confirmed the *whole pipeline actually worked correctly end to end* — the dispatch fired, the reducer stored it, `buildPdf` picked it up, `applyEdit` drew it. The apparent "text is missing" symptom had two separate, unrelated causes, both on the verification side: (1) `strings`/`grep` couldn't find the drawn text because pdf-lib Flate-compresses newly-added content streams by default (confirmed by loading the output back through pdf-lib itself and manually `zlib.inflateSync`-ing each of the page's now-multiple `/Contents` stream objects — the original, untouched content stream was uncompressed and grep-able, but every stream pdf-lib appended was compressed and read as binary garbage under `strings`), and separately (2) pdf-lib's `drawText` encodes its `Tj` operand as a hex string (`<4544...>`) rather than a literal parenthesized string for at least some of these draws, so even decompressed, a plain-text search for the decoded word wouldn't match the still-hex-encoded file bytes. Neither of these is a bug — it's exactly how pdf-lib serializes documents — but both had to be understood before the "no text found" signal could be trusted or dismissed.
  - Fixing the Playwright side (Playwright's `locator.selectText()` instead of a keyboard `Ctrl+A`) resolved the input-replacement problem cleanly.
- While chasing the above with instrumentation, the debug logging *did* surface one real, code-level bug, distinct from the test-script issues: `applyEdit`'s vertical-overflow handling used a hard clip (`if (lineY < y) return`) rather than growing the box, and because the sample PDF's text blocks were exactly one line tall (`height` equal to a single line's font size), *any* wrapped text beyond the very first line was silently dropped — the debug logs showed lines 2 and 3 of a 3-line wrap explicitly marked `willSkip: true`. This directly violated the plan's own "Done when" scenario for this phase ("editing a sentence inside one paragraph, including making it longer... shows the edited text in place, wrapped within its original block") — a longer sentence in a one-line block is the single most common case this feature needs to handle, and clipping silently dropped almost all of it. Rewrote the overflow policy to grow the occlusion-and-redraw box downward (keeping its top edge fixed, extending its bottom to fit `lines.length * lineHeight`) instead of clipping — one of the two options the plan explicitly left as an "implementation-time decision, informed by trying it against a real document," and the growing behavior is what that trial run actually called for.

### What I learned

- `strings`/`grep` against a PDF's raw bytes — the exact technique Steps 1-6 relied on repeatedly and successfully — stops being trustworthy the moment a workflow starts drawing *new* content into a document with a library that compresses what it writes. The failure mode isn't a clear error; it's simply not finding what should be there, which looks identical to "the feature is broken." The fix isn't to distrust `strings` generally (it remained reliable for the unmodified, pre-existing content in every phase up through Step 6), but to recognize when a *newly introduced write path* changes the file's own encoding and reach for a stronger tool (load it back through the same library that wrote it, and manually inflate the compressed streams) before concluding a feature doesn't work.
- Testing a wrapping/overflow behavior against a source document whose text blocks happen to be exactly one line tall is another version of Step 3's "don't test rotation against a square page" trap: a hard-clip bug and a working grow-the-box implementation would have looked *identical* against a source block tall enough to already fit 2-3 lines of the edited text, since neither would need to grow or clip in that case. The one-line-tall block wasn't a contrived edge case chosen to be adversarial — it's the ordinary case for short paragraphs and headings — and it was exactly what made this bug visible at all.

### What was tricky

Diagnosing the "is the text actually there" question required peeling back three separate layers that all looked like the same symptom (grep finds nothing) but had three unrelated causes: a test-script input bug (Ctrl+A not selecting), a pdf-lib serialization detail (Flate compression on new streams), and a pdf-lib encoding detail (hex-string `Tj` operands). Only after ruling out the first two by decompressing and hex-decoding the actual bytes was it possible to see the real, fourth thing that actually was a bug (the vertical clip). Jumping straight to "the feature must be broken" at any of the first three points would have led to fixing the wrong thing.

### What warrants review

- `matchStandardFont`'s heuristic (`/src/lib/pdfExport.ts`) is, per the plan's own framing, a known, explicit v1 limitation — edited text will not visually match the surrounding original text's exact typeface for any document using a non-standard or embedded font. This was not deeply tested against real-world documents with embedded custom fonts (only against this project's own Helvetica-based synthetic samples), so its practical accuracy on genuinely varied source PDFs is unverified.
- The "grow the box downward, don't clip" fix has no upper bound — an edit that grows extremely long (many wrapped lines) will keep extending the occlusion+redraw box downward with no cap, potentially drawing over unrelated content further down the same page if the edited block sits low enough. This wasn't specially guarded against, matching the plan's framing of overflow behavior as an accepted implementation-time judgment call rather than a spec to re-litigate — but it's a real, visible failure mode a reviewer should be aware of, not just a theoretical one.
- The block-clustering thresholds (`LINE_Y_EPSILON`, `BLOCK_LINE_GAP_FACTOR`, `BLOCK_LEFT_ALIGN_TOLERANCE` in `/src/lib/textBlocks.ts`) were tuned only against this session's two hand-built synthetic PDFs. The plan explicitly frames this as a first attempt to validate, not a fixed spec — real-world documents with justified/ragged-right text, mixed font sizes within one paragraph, multi-column layouts, or tighter/looser line spacing than these thresholds assume would all be worth trying before trusting this against production documents.
- The one-time, in-app disclosure banner (`hasSeenTextEditCaveat` state in `Workspace.tsx`, passed down to `PagePreview.tsx`) is session-scoped only (a plain `useState`, not persisted) — a user who reloads the page will see it again on their next edit. This was a deliberate simplicity choice (no `localStorage` plumbing for a v1 disclosure), consistent with the project's "nothing more than basic" framing, but worth confirming that's the intended durability for a disclosure this substantive.

### Future work

Phase 8 (image editing) shares this phase's exact architectural shape — edits as data appended to `WorkingPage.edits`, drawn only at `buildPdf` time, automatically undoable through the same history wrapper — and carries the identical occlusion-not-deletion caveat, which the plan explicitly says should be surfaced "as one combined 'what editing really means in this tool' disclosure," not a second, separate surprise. The `hasSeenTextEditCaveat` mechanism added this step should likely be generalized to cover both, rather than adding an independent second banner when Phase 8 lands.
